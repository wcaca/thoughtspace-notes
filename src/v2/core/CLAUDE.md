...
- `thought.js` — 念头/记忆实体（相变 5 态 + 材质 6 种 + 形状 6 种），EntityStateAttachment 契约工厂 + isEntityStateAttachment 校验（被 D001 强制调用），computeDisplayScale 近大远小，startPhaseTransition/tickPhaseTransition 瞬态相变（不持久化），toJSON/fromJSON Yjs 序列化
...
// v2-S2.1 thought.js: 2026-07-08 S2 念头实体第一个核心组件, 定义 EntityStateAttachment 契约
// v2-S2.8 thought.js: 2026-07-10 修 _validatePosition 调错 API (getLayerById → getLayer) + 错变量 (layerId → this.attachment.layerId)
