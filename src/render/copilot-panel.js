 ...
 * [POS]: src/render/copilot-panel.js — 灵感助手面板结构 + 事件 + 今日觉察(预览卡片渲染委托 copilot-preview.js)
 * [PROTOCOL]: 变更时更新此头部,然后检查 ../CLAUDE.md
 */

import { createClusterCard, createEdgeCard } from './copilot-preview.js';
...
export function showCopilotPanel(copilot, ctx) {
  ...
  let refresh;
  refresh = () => renderContent(content, copilot, ctx, root, refresh);
...
function renderContent(content, copilot, ctx, root, refresh) {
  content.innerHTML = '';
  ...
  if (clusters.length === 0) {
    ...
    for (const c of clusters) {
      content.appendChild(createClusterCard(c, ctx, refresh));
    }
  }
  ...
  if (edges.length > 0) {
    ...
    for (const e of edges) {
      content.appendChild(createEdgeCard(e, ctx));
    }
  }
}
